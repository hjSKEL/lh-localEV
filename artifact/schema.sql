...
CREATE TABLE `TB_CUCA001` (
  ...
  `STOP_RSN_CD` varchar(20) DEFAULT NULL COMMENT '정지사유코드(LOSS/USER_REQ/UNPAID/ETC)',
  `STOP_RSN_TXT` varchar(200) DEFAULT NULL COMMENT '정지사유 기타 직접입력(STOP_RSN_CD=ETC일 때만)',
  `REG_DT` datetime DEFAULT NULL COMMENT '등록일',
  `REG_ID` char(9) DEFAULT NULL COMMENT '등록자',
  `UPD_DT` datetime DEFAULT NULL COMMENT '수정일',
  `UPD_ID` char(9) DEFAULT NULL COMMENT '수정자',
  PRIMARY KEY (`CUT_CRD_NO`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='회원카드 — CUT_STAT_CD/LOS_ID/DEL_ID/DEL_DT 폐지, LOS_YN(분실만 구분)은 STOP_RSN_CD(분실/사용자요청/관리비미납/기타)로 확장(2026-08-27)';
