...
public class CertificateSignedBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.CertificateSigned response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.CertificateSigned.class);
            log.debug("CertificateSignedBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        }
    }
}
