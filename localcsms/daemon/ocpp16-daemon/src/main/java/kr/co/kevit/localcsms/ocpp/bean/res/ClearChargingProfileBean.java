...
public class ClearChargingProfileBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.ClearChargingProfile response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.ClearChargingProfile.class);
            log.debug("ClearChargingProfileBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        }
    }
}
