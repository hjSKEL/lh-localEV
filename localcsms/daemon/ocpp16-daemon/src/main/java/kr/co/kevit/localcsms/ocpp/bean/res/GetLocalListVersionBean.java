...
public class GetLocalListVersionBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.GetLocalListVersion response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.GetLocalListVersion.class);
            log.debug("GetLocalListVersionBean cpCsId={} getListVersion={}", cpCsId, response.getListVersion());
        }
    }
}
