...
public class InstallCertificateBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.InstallCertificate response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.InstallCertificate.class);
            log.debug("InstallCertificateBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        }
    }
}
