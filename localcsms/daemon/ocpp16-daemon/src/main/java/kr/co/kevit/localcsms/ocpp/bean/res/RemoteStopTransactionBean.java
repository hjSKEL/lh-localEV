...
@Component
public class RemoteStopTransactionBean implements ResponderBean {

    private static final Logger log = LoggerFactory.getLogger(RemoteStopTransactionBean.class);
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        kr.co.kevit.ocpp16.response.RemoteStopTransaction response =
                objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.RemoteStopTransaction.class);
        log.debug("RemoteStopTransactionBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
