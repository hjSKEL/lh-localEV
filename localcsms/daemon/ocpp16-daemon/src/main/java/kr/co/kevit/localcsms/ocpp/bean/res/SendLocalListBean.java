...
public class SendLocalListBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.SendLocalList response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.SendLocalList.class);
            log.debug("SendLocalListBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        }
    }
}
