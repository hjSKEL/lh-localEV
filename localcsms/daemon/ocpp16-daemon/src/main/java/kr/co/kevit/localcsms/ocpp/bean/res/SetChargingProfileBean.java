...
public class SetChargingProfileBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        kr.co.kevit.ocpp16.response.SetChargingProfile response =
                objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.SetChargingProfile.class);
        log.debug("SetChargingProfileBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        ...
        switch (response.getStatus()) {
            case Accepted:     datum.setEventCode("EVTR01"); break;
            case Rejected:     datum.setEventCode("EVTR02"); break;
            case NotSupported: datum.setEventCode("EVTR03"); break;
        }
