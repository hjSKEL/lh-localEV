...
public class UnlockConnectorBean implements ResponderBean {
    ...
    public void handle(String cpCsId, JsonNode payload, String uniqueId) throws Exception {
        if (log.isDebugEnabled()) {
            kr.co.kevit.ocpp16.response.UnlockConnector response =
                    objectMapper.treeToValue(payload, kr.co.kevit.ocpp16.response.UnlockConnector.class);
            log.debug("UnlockConnectorBean cpCsId={} getStatus={}", cpCsId, response.getStatus());
        }
    }
}
