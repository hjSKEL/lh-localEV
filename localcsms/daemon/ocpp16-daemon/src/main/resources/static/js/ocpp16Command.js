var ocpp16CommandJs = function () {
    ...
    function _UnlockConnector(params) {
        var obj = { connectorId: Number(params[0]) };// > 0
        return JSON.stringify(obj);
    }

    function _ChangeConfiguration(params) {
        var obj = { key: params[0], value: params[1] };// > 0
        return JSON.stringify(obj);
    }
    ...
    function _ChangeAvailability(params) {
        var obj = { connectorId: Number(params[0]), type: params[1] };//Operative, Inoperative
        return JSON.stringify(obj);
    }
    ...
    function _ClearChargingProfile(params) {
        var obj = {};
        if (params[0] != "") {
            obj.id = Number(params[0]);
        }
        if (params[1] != "") {
            obj.connectorId = Number(params[1]);
        }
        if (params[2] != "") {
            obj.stackLevel = Number(params[2]);
        }
        if (params[3] != "") {
            obj.chargingProfilePurpose = params[3];
        }
        return JSON.stringify(obj);
    }
    ...
    function _RemoteStartTransaction(params) {
        ...
        if (params.length > 2) {
            obj.chargingProfile = {
                chargingProfileId: Number(params[3]),//3
                stackLevel: Number(params[4]),//4
                chargingProfilePurpose: params[5],//5
                chargingProfileKind: params[6],//6
                chargingSchedule: {
                    chargingRateUnit: params[11],//11
                    chargingSchedulePeriod: params[15],//15
                    duration: Number(params[12]),//12
                    minChargingRate: Number(params[14])//14
                },
                validFrom: params[9],//9
                validTo: params[10]//10
            }
            if (params[7] != '0') {
                obj.chargingProfile.transactionId = Number(params[7])
            }
            if (params[8] != 'N/A') {
                obj.chargingProfile.recurrencyKind = params[8]
            }
            if (params[13] != '') {
                obj.chargingProfile.chargingSchedule.startSchedule = params[13]
            }
        }
        return JSON.stringify(obj);
    }
    ...
    function _SetChargingProfile(params) {
        ...
        if (params[5] != '0') {
            obj.csChargingProfiles.transactionId = Number(params[5])
        }
        if (params[6] != 'N/A') {
            obj.csChargingProfiles.recurrencyKind = params[6]
        }
        return JSON.stringify(obj);
    }
    ...
    function _makeParamDev(type, params) {
        switch (type) {
            ...
            case 'ChangeConfiguration':
                return _ChangeConfiguration(params);
            ...
            case 'GetConfiguration':
                return _GetConfiguration(params);
            case 'GetDiagnostics':
            ...
            case 'SignedUpdateFirmware':
                return _SignedUpdateFirmware(params);
            case 'SetChargeLimit': // for IIAC
                return _SetChargeLimit(params);
            case 'SetNetworkProfile': // for IIAC
    ...
    function _typeDev() {
        let typesDev = [
            ...
            'UpdateFirmware',
            'UnlockConnector',
            'SetChargeLimit',// for IIAC
            'SetNetworkProfile'// for IIAC
        ];
        return typesDev;
    }
