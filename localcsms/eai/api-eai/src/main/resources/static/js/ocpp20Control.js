...
var ocpp20ControlJs = function() {
	...
	function _initEvent() {
        ...
        $("#makerType").append('<option value="">선택</option>');
		var html = '';
	...
	function _addGetMonitoringReport(){
		...
		html += '<td><button onclick="ocpp20ControlJs.removeGetMonitoringReport(this);">삭제</button></td>';
		html += "</tr>";
	...
	function _searchEnvInfoOnClick(){
		...
		case 'CSUNIQID':
            if (sWord.length != 11) {
                toastr.warning("충전기QR는 11자리입니다.", "충전이력");
                return;
            }
            data.searchCond.csUniqId = sWord;
            _findCpCsId(sWord);
            break;
		case 'CHAID':
			if(!sWord.includes('-')){
	...
	function _searchEnvInfo(){
		...
		$.ajax({
			type: 'GET' ,
			url : _ctx + "/ws/charger/envInfo/cpCs/" + data.searchCond.cpCsId,
			dataType : 'json' ,
			success : function(jsonData, textStatus, jqXHR) {
	...
	function _searchCustRechargingOnClick(){

		custRechargingPageInfoJs.init('custRechargingPageInfoJs', 'CustRechargingPagingUl', 10, 5, ocpp20ControlJs.searchCustRecharging);
		data.searchCondRecharging.fromDate = formmatUtilsJs.removeDash($("#date3").val()) + "000000";
		data.searchCondRecharging.toDate = formmatUtilsJs.removeDash($("#date4").val()) + "235959";

		data.searchCondRecharging.csUniqId = "";

		let searchType = $("#searchType").val();
		let sWord = $("#sWord").val().trim();
		if(!sWord || sWord === ''){
			toastr.warning("검색조건을 입력하세요.","충전이력");
			return;
		}
		...
		case 'CSUNIQID':
			...
			data.searchCondRecharging.csUniqId = sWord;
			_findCpCsId(sWord);
			break;
		case 'CHAID':
			if(!sWord.includes('-')){
			...
			let cpCsIds = sWord.split("-");
			data.searchCondRecharging.cpId = cpCsIds[0];
			data.searchCondRecharging.csId = cpCsIds[1];
			_findCsUniqId(data.searchCondRecharging.cpId, data.searchCondRecharging.csId);
			break;
		default :
		}
	...
	function _searchCustRecharging(){
		//
		$("#Recharging_tBodyList").empty();
		let html = '<tr style="text-align:center;">';
		html += '<td colspan="12">검색 중입니다.</td>';
		$("#Recharging_tBodyList").append(html);

		let param = '';
		if (data.searchCondRecharging.csUniqId) {
			param += "?csUniqId=" + data.searchCondRecharging.csUniqId ;
		}
		param += "&fromDate=" + data.searchCondRecharging.fromDate;
		param += "&toDate=" + data.searchCondRecharging.toDate;
	...
	function _displayRecharging(jsonData){
		...
		if(!jsonData.result){
			...
			html += '<td colspan="12">검색된 데이터가 없습니다.</td>';
			html += '</tr>';
			$("#Recharging_tBodyList").append(html);
			return ;
		}
		...
		for(let i=0, length = result.length ; i < length ; ++i){
			...
			html += '<td>' + result[i].rechargingId+ '</td>';
   			if (parent.UserRole !== 'COMPANY_USER') {
   				...
   				$("#companyNameTh1").remove();
   				$("#companyNameTh2").remove();
   			}
			cardnum = result[i].cutCardNo;
			phnNo = result[i].mblPhoneNo;
			html += '<td>' + (result[i].custName ? result[i].custName : "-") + '</td>';
			html += '<td>' + (result[i].cutCardNo ? cardnum : "-") + '</td>';
//			html += '<td>' + (result[i].cutCardNo ? cardnum : "-") + '<i class="bi bi-files" style="cursor:pointer;" onclick="ocpp16ControlJs.doCustCardNo(' + result[i].cutCardNo + ')"></i></td>';
			html += '<td>' + (result[i].mblPhoneNo ? phnNo : "-") + '</td>';
			...
			html += '<td>' + (result[i].finalPaySum ? result[i].finalPaySum : "0") + '</td>';
	...
	function _displayEnvInfo(jsonData){
		...
		$("#fmwVer").val(jsonData.fmwVer);
		$("#cfgVersion").val(jsonData.cfgVersion);
		$("#serverConIp").val(jsonData.serverConIp);
		$("#serverConPort").val(jsonData.serverConPort);
		$("#fmwConIp").val(jsonData.fmwConIp);
		$("#fmwConPort").val(jsonData.fmwConPort);
		$("#sbyTranPeriod").val(jsonData.sbyTranPeriod);
		$("#csTranPeriod").val(jsonData.csTranPeriod);
		$("#swVersion").val(jsonData.swVersion);
		$("#lowVolt").val(jsonData.lowVolt);
		$("#protocolType").val(jsonData.protocolType);
		
		let GetDiagnosticsValue1 = $("#GetDiagnosticsValue1").val();
		$("#GetDiagnosticsValue1").val(GetDiagnosticsValue1.substring(0,GetDiagnosticsValue1.lastIndexOf('/')) + "/" + data.searchCond.cpCsId);
		
		if(jsonData.protocolType != 'OCPP20'){
			self.location= _ctx + "/charger/chargingStation/" + jsonData.protocolType + "/control?cpCsId=" + data.searchCond.cpCsId;
		}
	};
	
	function _searchOcppLogClick(){
		//console.log("_searchChargerStatusClick start....");
		ocppPageInfoJs.init('ocppPageInfoJs', 'OCPPPagingUl', 10, 20, ocpp20ControlJs.searchOcppLog);
	...
	function _addChargingSchedulePeriod(){
		...
		html += '<td><button onclick="ocpp20ControlJs.removeChargingSchedulePeriod(this);">삭제</button></td>';
		html += "</tr>";
	...
	function _addSetVariableMonitoring(){
		...
		html += '<td><input type="text"  class="form-control input-sm" /></td>';
		html += '<td><button onclick="ocpp20ControlJs.removeSetVariableMonitoring(this);">삭제</button></td>';
		html += "</tr>";
	...
	function _addSetVariables(){
		...
		let html = "<tr><td>";		
		html += '<select class="input-sm form-control input-s-sm inline" onchange="ocpp20ControlJs.changeSetVariableItem(this)">';
		...
		html += "</td>";
      	...
      	html += '</div></td>';
      	html += '<td><button onClick="ocpp20ControlJs.deleteSetVarialbeItem(this);">삭제</button></td>';
      	html += "</tr>";
	...
	return {
		...
		searchCustRecharging : _searchCustRecharging,
