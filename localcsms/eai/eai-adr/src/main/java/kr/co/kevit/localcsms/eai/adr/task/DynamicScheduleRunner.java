 ...
 * @since 2025. 1. 12.
 */
public class DynamicScheduleRunner extends Thread {

    private static final Logger LOGGER = LoggerFactory.getLogger(DynamicScheduleRunner.class);

    @Override
    public void run() {
        LOGGER.info("DynamicScheduleBean START");
        waiting(5000);
        do {
            if ("START".equals(DERStore.dynaic_status)) {
                sendMessage();
            }
        } while (waiting(10 * 1000));
        LOGGER.info("DynamicScheduleBean END");
    }

    private void sendMessage() {
        //
        DERStore.dynaic_status = "ING";
        try {
            LOGGER.info("DynamicScheduleBean sendMessage");
            // OCPP 2.1 UpdateDynamicScheduleRequest
            UpdateDynamicSchedule request = DERStore.getInstance().makeUpdateDynamicSchedule();
            StringBuilder builder = new StringBuilder(255);
            builder.append("412200001-03");
            builder.append(StringConstants.SEMICOLON);
            builder.append("SetChargingProfile");
            builder.append(StringConstants.SEMICOLON);
