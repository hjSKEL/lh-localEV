 ...
 * 등록된 충전기 전체 수만큼 {@code cpoCsmsAddress + "/" + cpCsId} 로 개별 세션(기존 로직).</li>
 * </ul>
 *
 * <p>
 * 운영 중 {@code ConnConfig}/충전기 추가·변경은 재기동으로 반영한다(동적 재조회 없음).
 * </p>
 *
 * @author bckim
 */
...
public class ChargerSessionManager {
    ...
    public void start() {
        ...
        startCs0(buildUrl(base, localSystemId));
        ...
        // CPO 모드 — 등록된 충전기 전체를 개별 세션으로 추가 기동(기존 로직).
        if (chargers.isEmpty()) {
            LOGGER.warn("[proxy-eai] 등록된 충전기가 없습니다 — CS0 세션만 유지합니다.");
            return;
        }
        ...
        for (ChargingStationDto cs : chargers) {
            ...
            try {
                ...
                ChargerSession session = new ChargerSession(cpCsId, buildUrl(base, cpCsId), props, sslSocketFactory,
        ...
        LOGGER.info("[proxy-eai] 충전기 세션 {}개 기동 완료 (CS0 포함 총 {}개)", sessions.size(), sessions.size() + 1);
    }

    private void startCs0(String url) {
    ...
    public void shutdown() {
        LOGGER.info("[proxy-eai] 세션 종료 중... (충전기 세션 {}개 + LH notify 구독 {}개 + CS0)", sessions.size(),
                lhListenerContainers.size());
