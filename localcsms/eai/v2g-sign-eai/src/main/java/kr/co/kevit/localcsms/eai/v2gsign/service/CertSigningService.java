...
/**
 * V2G X.509 인증서 서명 서비스 (ECDSA 전용).
 *
 * V2G PKI (ISO 15118) ECDSA CA 체인으로 인증서를 서명한다.
...
public class CertSigningService {
    ...
    @Value("${v2g-sign-eai.csr-save-path:/root/ca.v2g/intermediate/csr/}")
    private String csrSavePath;

    @Value("${v2g-sign-eai.cert-save-path:/root/ca.v2g/intermediate/cert/}")
    private String certSavePath;

    @Value("${v2g-sign-eai.cert-validity-days:375}")
    private int validityDays;

    @Value("${v2g-sign-eai.ocsp-url:http://ocsp.kevit.co.kr:8080}")
    private String ocspUrl;

    // ── V2G ECDSA CA 체인 (V2gSignEaiConfig 에서 주입) ────────────────────────
     ...
     *
     * 1. CSR 파일 저장
     * 2. V2G ECDSA CA 체인으로 인증서 서명
     * 3. 체인 PEM 구성 (충전기 인증서 + V2G SUB CA)
     * 4. 인증서 파일 저장
     * 5. 충전기로 전송 위임
     */
    @Async
    public void signAndSend(String csId, String certificateType, PKCS10CertificationRequest csr) {
        try {
            ...
            // 2. CA 체인 확인
            if (ecCaCert == null || ecCaKey == null) {
                log.error("[V2G-SIGN] csId={} V2G ECDSA CA 체인 미설정 — 서명 불가", csId);
                return;
            }

            // 3. 인증서 서명
            log.info("[V2G-SIGN] csId={} SHA256withECDSA 서명 시작", csId);
            String certPem = buildCertificate(csr, ecCaCert, ecCaKey);

            // 4. SUB CA 인증서를 체인에 추가
            String caCertPem = toPem(ecCaCert);
            String chainPem = certPem + caCertPem;
            log.info("[V2G-SIGN] csId={} 체인 구성 완료 (충전기 인증서 + V2G SUB CA)", csId);
            ...
            log.info("[V2G-SIGN] csId={} 인증서 저장: {}", csId, certFilePath);

            // 6. 전송 위임
            senderService.send(csId, certificateType, chainPem);

        } catch (Exception e) {
            log.error("[V2G-SIGN] csId={} 서명 처리 실패: {}", csId, e.getMessage(), e);
        }
    }
    ...
    private String buildCertificate(PKCS10CertificationRequest csr,
        ...
        X509v3CertificateBuilder builder = new JcaX509v3CertificateBuilder(
                caCert, serial, notBefore, notAfter,
        ...
        builder.addExtension(new ASN1ObjectIdentifier("2.16.840.1.113730.1.13"), false,
                new DERIA5String("KEVIT V2G Certificate"));
    ...
    private void saveCsr(String csId, PKCS10CertificationRequest csr) throws Exception {
        ...
        Files.writeString(Paths.get(csrFilePath), sw.toString(),
                ...
                StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
        log.info("[V2G-SIGN] csId={} CSR 저장: {}", csId, csrFilePath);
    }
}
