...
/**
 * TB_CHLC001 - 충전기 로컬 인증 목록 페이지 Controller
 */
@Controller
@RequestMapping("localCustomer")
public class LocalCustomerController {

    private static final Logger LOGGER = LoggerFactory.getLogger(LocalCustomerController.class);
    ...
    public ModelAndView list(HttpServletRequest req) {
        LOGGER.debug("/localCustomer/list");
        registerAccessLog(req, "/localCustomer/list");
        return new ModelAndView("charger/localCustomer/list");
    }

    @RequestMapping(value = "/detail", method = RequestMethod.GET)
    @Secured({"ROLE_ADMIN", "ROLE_OPER"})
    public ModelAndView detail(HttpServletRequest req, String cpId, String csId) {
        LOGGER.debug("/localCustomer/detail cpId={} csId={}", cpId, csId);
        registerAccessLog(req, "/localCustomer/detail");
        ModelAndView mav = new ModelAndView("charger/localCustomer/detail");
