 ...
 */
@RestController
@RequestMapping("ws/charger/cert")
public class CsCertResource extends AbstractResource {
    ...
    public Page<CsCert> searchCsCertList(CsCertSearchCond searchCond, HttpServletRequest request) {
        ...
        LOGGER.info("[REQ] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/search, GET, DATA : {}", loginUser.getUserId(), accessIp, new Gson().toJson(searchCond));
        Page<CsCert> resultSet = null;
        try {
            resultSet = csCertService.retrieveCsCertBySearchCond(searchCond);
            LOGGER.info("[RES] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/search, GET, SUCCESS", loginUser.getUserId(), accessIp);
        } catch (Exception e) {
            LOGGER.info("[RES] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/search, GET, FAIL", loginUser.getUserId(), accessIp);
    ...
    public CsCert searchCsCertDetail(@PathVariable("certId") BigInteger certId, HttpServletRequest request) {
        User loginUser = SessionManager.getLoginUser();
        String accessIp = getAccessIp(request);
        LOGGER.info("[REQ] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/detail/{}, GET", loginUser.getUserId(), accessIp, certId);
        CsCert result = null;
        try {
            result = csCertService.retrieveCsCert(certId);
            LOGGER.info("[RES] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/detail/{}, GET, SUCCESS", loginUser.getUserId(), accessIp, certId);
        } catch (Exception e) {
            LOGGER.info("[RES] USER ID :{}, ACCESS_IP:{}, URL : ws/charger/cert/detail/{}, GET, FAIL", loginUser.getUserId(), accessIp, certId);
            LOGGER.error(e.getMessage(), e);
        }
        return result;
    }
