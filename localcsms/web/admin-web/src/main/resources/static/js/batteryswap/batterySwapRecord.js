...
var batterySwapRecordJs = function () {
    ...
    function _init(requestId) {
        ...
        $("#btnList").click(function () { self.location = _ctx + "/batterySwap/record/list"; });
        $("#btnSlotHistory").click(function () {
            self.location = _ctx + "/batterySwap/slot/his/list?eventRefId=" + encodeURIComponent(state.requestId);
        });
    ...
    function _load() {
        $.ajax({
            type: 'GET',
            url: _ctx + "/ws/batterySwap/record/" + encodeURIComponent(state.requestId),
            dataType: 'json',
