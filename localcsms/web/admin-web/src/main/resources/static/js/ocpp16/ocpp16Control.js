...
let ocpp16ControlDevJs = function () {
    ...
    function _sendCommandOnClick() {
        ...
        if (!ocppCommandType) { alert(_msg.selectCmd); return; }
        let params = [];
    ...
    function _sendCommand(cmdType, valueStr) {
        ...
        if (!cpId) { alert(_msg.enterId); return; }
        ...
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>' + _msg.sending;

        _addLog('info', `[${cmdType}] → ${cpId}`);

        fetch('/ws/cmd/ocpp16/bypass/' + cpId, {
            method:  'POST',
        ...
        .then(function (result) {
            let success = result.ok && result.data && result.data.status === 'SUCCESS';
            _showResult(result.data, success);
            _addLog(success ? 'success' : 'fail',
                    '[' + cmdType + '] ' + (success ? _msg.success : _msg.fail));
        })
        .catch(function (err) {
            _showResult({ error: err.message }, false);
            _addLog('fail', '[' + cmdType + '] ' + _msg.error + ': ' + err.message);
        })
        .finally(function () {
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-send-fill me-2"></i>' + _msg.sendCmd;
        });
    }

    /* ── 세션 목록 ────────────────────────────── */
    function loadSessions() {
        fetch('/ws/cmd/ocpp16/bypass/sessions')
            ...
            .then(function (data) {
                ...
                if (sessions.length === 0) {
                    list.innerHTML = '<div class="text-center text-muted p-3 small">' + _msg.noConnected + '</div>';
                    if (countEl) countEl.textContent = _msg.connCount.replace('{0}', '0');
                    return;
                }

                if (countEl) countEl.textContent = _msg.connCount.replace('{0}', sessions.length);
    ...
    function _clearLog() {
        let panel = _id('logPanel');
        if (panel) panel.innerHTML = '<div class="text-center text-muted p-3 small log-placeholder">' + _msg.noLog + '</div>';
    }
    ...
    function _addChargingSchedulePeriod() {
        let html = '<tr>'
            ...
            + ' onclick="ocpp16ControlDevJs.removeChargingSchedulePeriod(this)">' + _msg.deleteBtn + '</button></td>'
            + '</tr>';
