 ...
 * @since 2019. 6. 30.
 */
public class RemoteStartTransactionBean  implements ControlerBean{
    
    private static final Logger LOGGER = LoggerFactory.getLogger(RemoteStartTransactionBean.class);
    ...
    public Object control(String csId, List<Object> res) {
        ...
        kr.co.kevit.ocpp16.response.RemoteStartTransaction response = gson.fromJson(text,kr.co.kevit.ocpp16.response.RemoteStartTransaction.class);
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("RemoteStartTransactionBean.getStatus : {}", response.getStatus());
        }
